...
if (~isempty(I2))
	y(I2) = h*sqrt(2*pi)*s1*normpdf(x(I2), c, s1);
end
if (~isempty(I3))
	y(I3) = h*sqrt(2*pi)*s2*normpdf(x(I3), c, s2);
end
...
x2 = m1;
y1 = 0;
y2 = h*sqrt(2*pi)*s1*normpdf(x2, c, s1);
sl1 = 0;
sl2 = -x2*h*sqrt(2*pi)*s1*normpdf(x2, c, s1); % Der. at x2.
